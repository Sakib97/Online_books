...
if (isset($_POST['comment_snd'])) {
  ...
  if (empty($comment)) {
    ...
    $insert_query = "INSERT INTO englishbook_comment (username ,bookname, comment, comment_time)
                    ...
                    header('location: english_book_page.php?e_id='.$e_id.'&status='.$status);
    ...
    ?>
       ...
       <div class="row">
         ...
         <div class="col-md-6">
           <p style="text-align: center;"> <strong> <?php echo $_SESSION['en_title']; ?> </strong> <br>
                ...
                <?php if ($status==1) { ?>
                    <a href="<?php echo $_SESSION['en_dl'] ?>" download>  <button class="btn"><i class="fa fa-download"></i>  Download</button> </a>
                                   <?php } else { ?>
                                     <br>
     ...
     <?php if ($status==1) { ?>
       <div class="container">
            ...
            <form class="" action="english_book_page.php?e_id=<?php echo $e_id; ?> &status=<?php echo $status; ?>" method="post">
